...
export default function TarjetaJuego({ game, onGameDeleted, onGameDetails, onGameEdited }: TarjetaJuegoProps) {
...
<Card className="py-4 h-auto sm:h-[700px] lg:h-[800px] flex flex-col">
  <CardHeader className="pb-0 pt-2 px-4 flex-col items-start flex-shrink-0">
    <p className="text-tiny uppercase font-bold line-clamp-1">{game.title}</p>
  ...
  <div className="flex flex-col gap-2 px-4 pb-4 mt-auto">  {/* ← mt-auto empuja abajo */}
    <Button className={`w-full ${game.isFavorite ? "bg-slate-600 hover:bg-red-500" : "bg-slate-500 hover:bg-purple-600"}`} onPress={handleFavorite}>
  ...
  <EditModal 
    ...
    onSuccess={() => {setIsEditModalOpen(false); onGameEdited?.();}} 
  />
</Card>
);
}